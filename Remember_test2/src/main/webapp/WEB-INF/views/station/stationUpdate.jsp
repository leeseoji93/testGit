...
%>
	...
	<jsp:include page="../include/menu_admin.jsp">
		...
		<jsp:param name="pageTitle" value="정류장 수정"  /> 
	</jsp:include>
	<div class="page">
		<div class="page-wrapper">
	
			<div class="pageTitle"><h2>정류장 수정</h2></div>
			
			<div class="pageContent700 textAlign-C">
				<!-- STEP2. 정류장명 입력 -->
				<div class="stepTitle">
				...
				<!-- STEP1. 정류장명 입력 -->
				<form:form action="stationUpdateSave" id="STForm" commandName="STForm" method="POST">
					<form:hidden path="latitude" value="${sessionStation.latitude}" />
					<form:hidden path="longitude"  value="${sessionStation.longitude}" />

				<div class="stepContent stationUpTable">
					<table>
						<tr>
							<th>정류장 명</th>
							<td><form:input path="station_name" value="${sessionStation.station_name}" id="station_name" cssClass="station_name big_input" /></td>
						</tr>
						<tr>
							<th>사용여부</th>
							<td>
								<c:choose>
			                        <c:when test="${sessionStation.station_use eq 1}">
			                        	<div class="toggleWrapper toggleleft6">
					                		<input type="checkbox" id="station_use" class="station_use"  name="station_use" checked="checked" value="${sessionStation.station_use}" onclick="stationUseClick(this.id)"/>
											<label for="station_use" class="toggle"><span class="toggle__handler"></span></label>
					                	</div>
			                        </c:when>
			                        <c:otherwise>
			                        	<div class="toggleWrapper toggleleft6">
					                		<input type="checkbox" id="station_use" class="station_use" name="station_use" value="${sessionStation.station_use}" onclick="stationUseClick(this.id)" />
											<label for="station_use" class="toggle" ><span class="toggle__handler"></span></label>
					                	</div>
...
<%-- 					<form:hidden path="registrant" value="<%=princial %>"/> --%>
					<form:hidden path="station_id" id="station_id" value="${sessionStation.station_id}"/>

					<form:hidden path="station_addr"  value="${sessionStation.station_addr}" />
				</div>
...
function stationUseClick(thisId){
	var checVal =  $("#"+thisId).val();
	console.log("chk_val : " + checVal);
	if(checVal==1){
		$('#'+thisId).val('0');	
	}else{
		$('#'+thisId).val('1');
...
//  		var stationName=${sessionStation.station_name};
		$("#STForm").validate({
		    	...
		    	station_name : {
		    		...
		    		remote: {
	    				...
	    				data: {
	    					...
	    					baseData: function(){
	    						return "${sessionStation.station_name}";
	    					}
	    				},
