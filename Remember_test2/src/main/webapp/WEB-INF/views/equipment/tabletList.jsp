...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
	<jsp:include page="../include/menu_admin.jsp">
		...
		<jsp:param name="pageTitle" value="테블릿조회"  /> 
	</jsp:include>
	
	<div class="page">
		<div class="page-wrapper">
			<div class="pageTitle"><h2>테블릿 조회</h2></div>
	
			<div class="pageContent700">
				<div class="searchBox741">
					<input type="text" name="keyWord" id="keyWord">
					<input type="hidden" value="테블릿" name="equipment_type" id="equipment_type">
				...
				<div class="categoryContent borderRadius">
					...
					<table id="equiTitle">
						<tr>
							<th width="233">기기명</th>
							<th width="169">관리자</th>
							<th width="149">사용여부</th>
							<th width="179">물품상태</th>
						</tr>
					</table>
...
<%-- 							 onclick="goDetail()DetailInfo('${dto_b.beacon_id}','${s.count}')"  onmouseenter="DetailInfo('${dto_b.beacon_id}','${s.count}')" --%>
							<c:choose>
								<c:when test="${fn:length(tabletList) eq 0}">
									<tr><td align="center">등록된 테블릿이 없습니다.</td></tr>
								</c:when>
								
								<c:otherwise>
									<c:forEach items="${tabletList}" var="dto_b" varStatus="s">
									<tr id="tabletList${s.count}" class="tr_hover">
										<td width="233">${dto_b.equipment_id}</td>
										<td width="169">${dto_b.staff_name}</td>
										<td width="149">${dto_b.equipment_use}</td>
										<td width="179">${dto_b.equipment_stat}</td>
									</tr>
									</c:forEach>
								</c:otherwise>
							</c:choose>	
...
var header = $("meta[name='_csrf_header']").attr("content");
	...
	$(document).ready(function(){	
		$("#tabletList1").trigger("click");
	});
	//Tr 클릭 하면 상세 조회 뜨게하기
	$(document).on('click', '#equiTable tr', function(){
		console.log("#equiTable tr click");
		var tr = $(this);
		var td = tr.children();
		
		var equipment_id = td.eq(0).text();			//비콘 값 (첫번째 td)
		var index = $("#equiTable tr").index(this); //몇번째 줄인지 알기 위함
		index +=1;
		var has = $('.tr_hover').hasClass("Listactive");
		var hoverRow = "#tabletList"+index;  		// 클릭 한 table의 row 
		...
		$.ajax({  
               ...
               success: function(data) {           	   
                 ...
                 for(var i=0; i < size_data.length; i ++){
                	 $("#equiTable")
                	 .append( 	"<tr id='tabletList"+(i+1)+"' class='tr_hover'>"
								+"<td width='234'>"+data.equipmentResult[i].equipment_id+"</td>"
								+"<td width='168'>"+data.equipmentResult[i].staff_name+"</td>"
								+"<td width='150'>"+data.equipmentResult[i].equipment_use+"</td>"
								+"<td width='178'>"+data.equipmentResult[i].equipment_stat+"</td>"
								+"</tr>");
                	 $("#tabletList1").trigger("click");
                 }
