...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
	<jsp:include page="../include/menu_admin.jsp">
		...
		<jsp:param name="pageTitle" value="게이트웨이조회"  /> 
	</jsp:include>
	
	<div class="page">
		<div class="page-wrapper">
			<div class="pageTitle"><h2>게이트웨이 조회</h2></div>
	
			<div class="pageContent700">
				<div class="searchBox741">
					<input type="text" name="keyWord" id="keyWord">
					<input type="hidden" value="게이트웨이" name="equipment_type" id="equipment_type">
...
<%-- 							 onclick="goDetail()DetailInfo('${dto_b.beacon_id}','${s.count}')"  onmouseenter="DetailInfo('${dto_b.beacon_id}','${s.count}')" --%>
							<c:choose>
								<c:when test="${fn:length(gateway_List) eq 0}">
									<tr><td align="center">등록된 게이트웨이가 없습니다.</td></tr>
								</c:when>
								
								<c:otherwise>
									<c:forEach items="${gateway_List}" var="dto_b" varStatus="s">
									<tr id="gatewayList${s.count}" class="tr_hover">
...
var header = $("meta[name='_csrf_header']").attr("content");
	...
	$(document).on('click', '#equiTable tr', function(){ 
		...
		var hoverRow = "#gatewayList"+index;  		// 클릭 한 table의 row 
	...
	$.ajax({  
              ...
              success: function(data) {           	   
                ...
                for(var i=0; i < size_data.length; i ++){
               	 $("#equiTable")
               	 .append( 	"<tr id='gatewayList"+(i+1)+"' class='tr_hover'>"
